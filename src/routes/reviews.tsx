...
export const Route = createFileRoute("/reviews")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Client Reviews | Vertex Property Maintenance Inc." },
      {
        property: "og:description",
        content: "4.9/5 average rating across 480+ reviews from commercial and residential clients.",
      },
...
function ReviewsPage() {
  ...
  return (
    <>
      ...
      >
        <div data-reveal data-reveal-delay="220" className="reveal mt-12 flex flex-wrap gap-10">
          {[
            ...
            <div key={s.k}>
              ...
              <p className="mt-1 text-xs uppercase tracking-widest text-white/45">{s.k}</p>
            </div>
          ))}
        </div>
      </PageHero>
            ...
            >
              ...
              <div className="flex gap-1">
                {Array.from({ length: r.rating }).map((_, k) => (
                  <Star key={k} className="h-4 w-4 fill-ember text-ember" />
                ))}
              </div>
              <blockquote className="mt-5 leading-relaxed text-foreground/85">"{r.quote}"</blockquote>
              <figcaption className="mt-7 border-t border-border pt-5 text-sm">
                <span className="block font-display font-extrabold">{r.name}</span>
      ...
      <section className="border-t border-border bg-muted py-20">
        <div data-reveal className="reveal mx-auto max-w-3xl px-6 text-center">
          <h2 className="font-display text-3xl font-black md:text-5xl">Join them.</h2>
